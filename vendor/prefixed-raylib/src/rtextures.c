...
RL_Image RL_LoadImageSvg(const char *fileNameOrString, int width, int height)
{
    ...
    if (fileNameOrString != NULL)
    {
        ...
        if (isSvgStringValid && (fileData != (unsigned char *)fileNameOrString)) RL_UnloadFileData(fileData);
    }
#else
...
#else
    TRACELOG(LOG_WARNING, "IMAGE: RL_GenImageText() requires module: rtext");
    image = RL_GenImageColor(width, height, RL_BLACK);     // Generating placeholder black image rectangle
#endif
